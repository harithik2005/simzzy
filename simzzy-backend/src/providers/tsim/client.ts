...
import { TSIM_SUCCESS_CODE, type TsimDataplan, type TsimResponse } from './types'
...
export class TsimClient {
  ...
  /** Read-only device/usage detail — `POST /tsim/v1/deviceDetail`. */
  deviceDetail(deviceId: string) {
    return this.request<unknown>('POST', '/tsim/v1/deviceDetail', { body: { device_id: deviceId } })
  }

  /** Read-only order/topup detail — `POST /tsim/v1/topupDetail`. */
  topupDetail(topupId: string) {
    return this.request<unknown>('POST', '/tsim/v1/topupDetail', { body: { topup_id: topupId } })
  }
}
